...
class Extension {
	static public function getText(key:String):String
		return (FileSystem.exists(key)) ? File.getContent(key) : null;
	
    static public overload extern inline function int(f:Float):Int 
        return Std.int(f);

	static public overload extern inline function int(f:String):Int 
        return Std.parseInt(f);
	
	static public function capitalize(text:String)
